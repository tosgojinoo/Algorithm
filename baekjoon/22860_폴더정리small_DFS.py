...
def DFS(name):
    files = [] # only file names under one folder
    ...
    for lowers in doc[name]:
        nname, isFolder = lowers
        if isFolder == '0': # 파일일 경우
...
doc = defaultdict(list) # {upper folder: [lower, isFolder for file of folder]}
...
for _ in range(N + M):
    ...
    if upper not in doc:
        doc[upper] = []
    doc[upper].append([lower, isFolder])
